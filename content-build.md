...
| Artists rotation rule (new, 2026-08-24) | Rotate future Artists posts by genre bucket, working through each of the 10 real genre files in `data/artists/` rather than picking names off the flat gap list in whatever order. Country started with Dolly Parton. | Charlie's direct instruction this session. Next country candidates from the gap list: Willie Nelson, Carole King, Jim Croce (Carole King and Jim Croce need their own genre-file check, may not actually be tagged `country`). After country is exhausted, move to the next genre bucket. Funk candidates (George Clinton, Rick James) and the remaining zero-coverage gaps (Marvin Gaye, Al Green, Donna Summer, Led Zeppelin, Pink Floyd, Yes, Genesis) still need sorting into this genre-rotation order before their next turn comes up. |
...
| Artists | 16 | Dolly Parton published 2026-08-24, first post under the new genre-rotation rule (country). Thin relative to the 601-artist JSON pool — real tool dead-end risk, see Sitemap-Verified Findings above |
