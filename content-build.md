...
# Rotation Queue slots 1-3 (70s Soft Rock, Queen, 70s Country Rock) shipped 2026-08-09 — next up is slot 1, Disco Music of the 70s (Genres).
...
| 1 | Genres | Disco Music of the 70s (already Planned, keyword already set, 65 real artists, Dancing Queen and Village People both waiting to link to it) | pending |
| 2 | Artists | Eagles (closes Hotel California link-owed gap; verify NFL-team search ambiguity first — the Queen keyword check found the site's own risk assumptions aren't always right, confirm with a real SERP/volume check, don't assume Eagles is safer just because it was flagged as such) | pending keyword check |
| 3 | Genres | 70s Hard Rock (68 artists) | pending keyword check |
| 4 | Artists | ABBA (closes Dancing Queen link-owed gap) | pending keyword check |
| 5 | Genres | Classic Rock of the 70s (66 artists — CHECK FIRST whether this genuinely differs from the already-live "70s Rock" post or is a data-labeling overlap before building; don't duplicate) | needs a content decision, not just a keyword check |
| 6 | Trivia | Next Trivia post drawing from the untapped 50-question QUIZ_POOL in js/quiz-questions.js | needs a content decision on format |
| 7 | Genres | 70s Funk (50 artists; check against the live Soul post for overlap first) | pending keyword check |
| 8 | Genres | Progressive Rock in the 70s (50 artists; fold in "concept albums" as a section) | pending keyword check |
| 9 | Genres | Pop Crossover of the 70s (50 artists — decide if this is distinct from Soft Rock/Yacht Rock or overlapping before building) | needs a content decision |
| 10 | Genres | 70s Punk Rock (real count closer to 23 once new-wave contamination is filtered out; can't lean on chart-hit hooks, zero punk songs in the chart data) | pending keyword check |
| 11 | Genres | Glam Rock in the 70s (no dedicated bucket — must be manually curated from hard-rock's 10 and pop-crossover's 7 subgenre-tagged artists) | needs manual curation before writing, not a simple pull |
...
| Genres | 4 live (zero placeholders) + 1 Planned | 70s Soft Rock and 70s Country Rock both shipped 2026-08-09; still the category with the most real material left to draw from |
