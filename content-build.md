...
# Current status: Stevie Wonder (Artists) is LIVE (2026-08-23, pushed and confirmed on the real site). Everything else completed to date is in the archive.
...
|---|---|---|
| Artists | Stevie Wonder | **LIVE 2026-08-23** — https://musicofthe70s.net/blog/artists/stevie-wonder/. Focus keyword `Stevie Wonder`, given directly by Charlie; confirmed never used as a focus keyword on this site before. Angle: the 1971 Motown contract renegotiation (21st-birthday leverage, Black Bull Music, full creative control) as the single pivot that triggered the TONTO/Malcolm Cecil-Robert Margouleff partnership, the five-album classic-period run, and three consecutive Album of the Year Grammys, closed out with all 10 of Wonder's Hot 100 year-end chart entries pulled directly from `data/billboard/year_end_hot100.json` (site's own ground truth). Distinct structure (single-decision-as-engine) from all 14 prior Artists posts. `verify_post.py` needed one real iteration: sentence-length distribution and a keyword-in-consecutive-sentences bug (image caption + next paragraph both carrying the exact phrase, plus a FAQ item boundary) both traced to the same `<p>`-tag-adjacency pattern flagged in the 2026-08-08 session log entry; fixed by splitting long sentences and pronoun-izing one paragraph each side. Passed clean after that (1,455 words, density 1.512%). One banned-word flag left standing on purpose: "journey" appears only inside the real album title *Journey Through the Secret Life of Plants*, can't be altered without falsifying the title. Mobile check (375px/390px) passed via real Playwright rendering, no horizontal overflow, TOC/FAQ/related-posts all stack correctly. Video: "Superstition (Single Version)" on the YouTube-auto-generated "Stevie Wonder - Topic" channel (licensed audio, same class of substitute used for Grateful Dead's audio-only embed), oEmbed-verified. Image: Stevie Wonder in a Los Angeles recording studio, Aug 1975 (UCLA Digital Library via Wikimedia Commons, CC BY 4.0), cropped to site's 43:24 ratio, 121KB. Category cards, blog hub, sitemap regenerated and confirmed live. **15th Artists post.** |
