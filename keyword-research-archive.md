# KEYWORD-RESEARCH-ARCHIVE.md — musicofthe70s.net (append-only, grep-only)
# Not read whole. Grep for a term: grep -i "mamma mia" keyword-research-archive.md
# Holds keyword pulls whose focus keyword is now on a Live post. New shipped pulls get
# pruned here from keyword-research-log.md when that file nears 195 lines (docs/file-rotation.md).


---

## Pruned from keyword-research-log.md 2026-09-02 (line-cap rotation)

Every pull below has its focus keyword on a Live post. Moved verbatim to keep the living file under 195 lines. Grep here for a past pull; the living file keeps only open/undecided material.
...
## ABBA — pull for Rotation Queue slot, 2026-08-10

Source: WordStream, supplied by Charlie, 2026-08-10, search term "ABBA". A prior real SERP check this session (bare-search, no volume tool) found bare "ABBA" is dominated by the band itself, no monarchy-style or NFL-style collision like Queen or Eagles; this pull confirms that with real volume.

### Real (genuinely about the band)

| Term | Volume/mo | Status |
|---|---|---|
| ABBA | 301,000 | **Confirmed focus keyword** — used on the ABBA Artists post, 2026-08-10 |
| ABBA Dancing Queen | — | Secondary keyword, logged against the existing Dancing Queen post specifically |
| Mamma Mia | 301,000 | Future post candidate — a Songs post; tied with the confirmed ABBA keyword itself in raw volume |
| Agnetha Fältskog | 49,500 | Future post candidate — solo Artists post |
| Anni-Frid Lyngstad | 27,100 | Future post candidate — solo Artists post |
| Bjorn Ulvaeus | 22,200 | Future post candidate — solo Artists post |
| Benny Andersson | 18,100 | Future post candidate — solo Artists post |
| Gimme Gimme Gimme | 33,100 | Future post candidate — a Songs post |
| Super Trouper | 9,900 | Future post candidate — a Songs post |
| ABBA Voyage | 8,100 | Future post candidate — a legacy-feature angle, distinct from a standard decade-scoped post |
| ABBA Queen | 135,000 | Signal for a possible future comparison/listicle post pairing the two bands; not a direct target for any single post |
...
## Four-Song Batch — keywords confirmed directly with Charlie, 2026-08-28

Charlie asked for the Songs category to get fresh material, no specific keywords named up front. Six candidates pulled from `data/billboard/year_end_hot100.json` top ranks with real story potential (Bridge Over Troubled Water, Stayin Alive, I Will Survive, Y.M.C.A., Seasons in the Sun) and checked via WebSearch for what's currently ranking; no volume tool run this session (WordStream not supplied), same as most prior batches. All 5 checked are dominated by generic "story behind the song" pages, none using real Billboard year-end data. Proposed a batch of 4, holding back `Y.M.C.A.` over real overlap risk with the live Village People Artists post (same act, same disco-era territory). Charlie confirmed all 4 literally: `bridge over troubled water`, `stayin alive`, `i will survive`, `seasons in the sun`.

**Literal-phrasing note**: `stayin alive` was specified without the apostrophe the real song title carries. Used the literal no-apostrophe phrase in every scored location (title, meta, first 100 words, H2, FAQ questions), per the standing literal-keyword-phrasing rule; the natural apostrophe form (`Stayin' Alive`) used only in flowing body prose. Slugs: `bridge-over-troubled-water`, `stayin-alive`, `i-will-survive`, `seasons-in-the-sun`, all exact word-order matches.
